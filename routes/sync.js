...
var syncHelper = require('../helpers/sync');
...
router.get('/syncChunk', function(req, res, next) {
  // afterUSN is optional. zero = full sync
  var afterUSN = parseInt(req.query.afterUSN) || 0;
  // maxEntries is optional. zero = unlimited
  var maxEntries = parseInt(req.query.maxEntries) || 0;
  // record this now so if an update happens before finished getting records,
  // updating fullSyncBefore will still force a full sync on this client
  var lastSyncTime = new Date()
  // get flat list of all records, ordered by ascending USN
  syncHelper.getOrderedRecordList(afterUSN, maxEntries, function(err, records) {
    if (err) {
      next(err);
    } else {
      // serialize all records to json, including classname and send package 
      // with metadata to client
      res.status(200).json({
        chunkHighUSN: records[records.length - 1].usn,
        lastSyncTime: lastSyncTime,
        records: records.map(function(record) {
          var json = record.toJSON();
          json.classname = record.Model.name;
          return json;
        })
      });
    }
